...
function onEncodeContent(index: number) {
  ...
  outputList.value.splice(index, 1, encodeURIComponent(inputList.value[index]));
...
function onDecodeContent(index: number) {
  ...
  let result = "";
  try {
    result = decodeURIComponent(inputList.value[index]);
    message.success("解码成功");
    isDecodeError.value = false;
  } catch (error: any) {
    result = `请检查 URL 编码格式是否正确\n解码失败:${error.message}`;
...
<template>
  <div class="url-root row">
    <div
      class="url-container auto-fill column"
    ...
    >
      <div class="auto-fill">
        <a-textarea
          ...
          style="height: 100%"
          placeholder="输入任意内容进行 URL 编码，或输入 URL 编码进行解码"
        />
      </div>
      ...
      <div class="auto-fill" style="margin-bottom: var(--space-lg)">
        <a-textarea
          ...
          placeholder="此处显示 URL 编码或 URL解码结果"
          readonly
        />
      </div>
    </div>
